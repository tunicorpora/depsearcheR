...
matched_sentences[1] %>% ConllAsTibble
...
So usually it's "man" who does something in the gospel of John. 
This is, however, so far not not 

```{r}


gospel_of_john_sentences[3]  %>% ConllAsTibble

test <- function(sentence){
    sentence %>% 
...
ApplyConllFilter(js,test, "matches") %>% 
    count(feat) %>% 
    arrange(desc(n))

jshead <- ApplyConllFilter(gospel_of_john_sentences,test, "matches")

js <- ApplyConllFilter(gospel_of_john_sentences,function(x){x  %>% })

gospel_of_john_sentences[3]  %>% 
    FilterConllRows("feat", "NN") %>% 
    GetDeps(gospel_of_john_sentences[3])  %>% 
    GetHeads(gospel_of_john_sentences[3])  

    FilterConllRows("dep", "ROOT")
funct <- function(s){
    first
}
