...
a similar filter. This set of sentences is included with the package
as a vector called `gospel_of_john_sentences`
...
start.time <- Sys.time()
matched_sentences <- Filter(AnotherFilter, gospel_of_john_sentences)
...
the whole New testament. That means 9,329 sentences (not included with this
package).
...
even for some more serious text mining work.
...
filters. Let's move on to imagining a scenario in which
...
the cases where a finite verb had a subject dependent:
...
This actually gives us `r length(matched_sentences)` sentences.
What if we want to, say, list the lemmas of the different subjects?
...
ConllSetAsTibble <- function(sentences) {
    indices <- seq_along(sentences)
    lapply(indices, function(idx){
        tib <- ConllAsTibble(sentences[idx]) %>% 
            mutate(sent_id=idx)
    }) %>% 
        do.call(rbind,.)  %>% 
        return
...
#my_result_set <- ConllSetAsTibble(matched_sentences)
...
newmatches <- ApplyConllFilter(gospel_of_john_sentences, AnotherFilter, "both_pretty")
#rawmatches <- ApplyConllFilter(gospel_of_john_sentences, AnotherFilter, "raw")
#newmatches  %>% count(lemma) %>% arrange(desc(n))
#newmatches %>% mutate(sent = rawmatches) -> newmatches


#my_result_set %>% AnotherFilter
#    filter(dep=="nsubj") %>% 
