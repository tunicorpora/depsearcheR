...
ApplyConllFilter_df <- function(tab, funct, sent_col){
...
The first way is to turn our filter function to something that returns
a truthy value instead of the matched rows. Look at the following example:


```{r}

ref_filt_truthy <- function(sentence){
    sentence  %>% 
        FilterConllRows("lemma", "referee") %>% 
        FilterConllRows("dep", "dobj") %>% 
        GetHeads(sentence) %>% 
