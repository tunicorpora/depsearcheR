...
called `ApplyConllFilter_df`. It's first argument is a tibble (or a data frame)
...
did these kinds of tweets come from:
...
We can also spesify, if
we want our data set filter to return an additional column 
...
ApplyConllFilter_df <- function(tab, funct, sent_col, return_col=""){
    if(return_col != ""){
        ...
        mt2 <- mtab %>% left_join(tab, by="parsed_text") 
        return(mt2)
    }
    return (tab)
}

temp <- ucl_ref %>% ApplyConllFilter_df(ref_filt, "parsed_text", "lemma")  


```

The modified filter returns the number of matched rows: if this is more than zero,
the value is considered *truthy*, otherwise it's *falsy*. These kinds of filters
could be used the following way:

```{r}

ref_as_object <- ucl_ref  %>% 
    filter(ref_filt_truthy(parsed_text))
